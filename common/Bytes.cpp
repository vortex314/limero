...
int Bytes::insert(uint32_t offset, uint8_t* data, uint32_t length) {
...
int Bytes::insert(uint32_t offset, Bytes& data) {
...
*/
int Bytes::write(uint8_t* data, int offset, int length) {
    for (int i = 0; i < length; i++) {
        int erc;
...
int Bytes::write(Bytes* data) { return write(data->_start, 0, data->_limit); }

int Bytes::read(uint8_t* data) {
    if (_offset < _limit)
        *data = _start[_offset++];
    else
        return EAGAIN;
