...
CborWriter &CborWriter::map()
{
  ...
  _error = cbor_encoder_create_map(_encoder, mapEncoder, CborIndefiniteLength);
  _encoders.push_back(_encoder);
  _encoder = mapEncoder;
  return *this;
}
...
CborError CborWriter::error() { return _error; }
...
CborWriter &CborWriter::add(int64_t v)
{
  if (_error == CborNoError)
  {
    _error = cbor_encode_int(_encoder, v);
...
CborWriter &CborWriter::addCrc()
{
  ...
  if (sz > (_capacity - 5))
  {
    WARN(__FILE__ " : not enough space for CRC");
  }
  else
  {
...
bool CborWriter::ok() { return _error == CborNoError; }
